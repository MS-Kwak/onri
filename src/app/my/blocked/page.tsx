    ...
    <div className="flex min-h-dvh flex-col bg-navy">
      <header className="sticky top-0 z-40 bg-navy">
        <div className="flex items-center gap-2 px-5 pt-12 pb-3">
          ...
          <h1 className="text-lg font-bold text-cream">차단 목록</h1>
        </div>
      ...
      <div className="flex flex-1 flex-col px-5 pt-5 pb-10">
        {blockedUsers.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-3">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-cream/5">
              <ShieldOff size={28} className="text-cream/15" />
            </div>
          ...
          <div className="flex flex-col gap-2">
            <p className="mb-1 text-xs text-cream/30">
              총 {blockedUsers.length}명
            </p>
            {blockedUsers.map((user) => (
              <div
                key={user.id}
                className="flex items-center justify-between rounded-2xl bg-cream/3 px-4 py-3.5"
              >
                ...
                <button
                  ...
                  className="flex items-center gap-1 rounded-lg bg-cream/5 px-3 py-1.5 text-xs font-medium text-cream/50 transition-colors hover:bg-cream/10 hover:text-cream/70"
                >
                  <UserX size={13} />
                  해제
                </button>
